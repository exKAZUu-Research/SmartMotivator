...
module.exports = function(j) {
  ...
  const findReactCreateClass = path =>
    path.findVariableDeclarators().filter(decl => findReactCreateClassCallExpression(decl).size() > 0);
  ...
  const createCreateReactClassCallExpression = properties =>
    j.callExpression(j.memberExpression(j.identifier('React'), j.identifier('createClass'), false), [
