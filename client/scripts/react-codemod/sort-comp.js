...
module.exports = function(fileInfo, api, options) {
  ...
  if (options['explicit-require'] === false || ReactUtils.hasReact(root)) {
    ...
    if (createClassSortCandidates.size() > 0 || es6ClassSortCandidates.size() > 0) {
...
function getMethodsOrder(fileInfo, options) {
  return options.methodsOrder || getMethodsOrderFromEslint(fileInfo.path) || defaultMethodsOrder;
}
